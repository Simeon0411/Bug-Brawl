    ...
    def movement(self, UP, LEFT, DOWN, RIGHT, ATTACK, SHIELD, SUPER, SPECIAL):
        ...
        if ATTACK and SHIELD and self.frame > (self.maxFrame - 3) and self.state != 11 or self.SPECIAL and self.state != 11:
            if self.state != 9:
                ...
                self.frame = 20
        # Shield 
        if self.grounded == 1 and self.state != 11 and self.state != 9:
    ...
    def collide(self):
        ...
        if collided:
            if self.VY > (CharacterGravity[self.character] * 20):
                self.VY *= 0.8
            if self.frame > 0 and self.VY > 3 and self.state != 10 and self.state != 11 and self.state != 12:
                self.frame = 0
        while any(self.hitbox.colliderect(rect) for rect in block):
            self.Y -= 1 if self.VY > 0 else -1
            self.hitbox = pygame.Rect(self.X, self.Y, self.sizeX, self.sizeY)
...
while running:
    ...
    for pl in players:
        ...
        if not(pl.flight == CharacterFlight[pl.character]) and pl.grounded == 0 and pl.wallGrab == 0:
            pygame.draw.rect(screen, (0, 0, 0), pygame.Rect(pl.X + pl.hitbox.width / 2 - pl.flight, pl.Y - 75 / 2, pl.flight * 2, 25))
    ...
    for proj in projectiles:
        if proj.type == 2 and (proj.frame // 2) % 2 == 0 and proj.frame < 21:
            ...
            pygame.draw.rect(screen, (0, 0, 0), proj.attack)
    for blk in block:
    ...
    for pl in players:
        ...
        screen.blit(pygame.transform.flip(bbbSprites[pl.animation()[0]], True if pl.facing == -1 else False, False), ((pl.X + (pl.animation()[1] * pl.facing), (pl.Y - 28))))
    pygame.display.flip()
    print(players[0].state < 13)
pygame.quit()