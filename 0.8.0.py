...
class player():
    ...
    class projectile:
        def __init__(self, X, Y, VX, VY, FX, FY, frame, sender, damage, angleX, angleY, direction, launch):
            self.X = X
            self.Y = Y
            self.VX = VX
            self.VY = VY
        ...
        def update(self):
            ...
            self.VY *= self.FY
            self.attack = pygame.Rect(self.X, self.Y, 10, 10)
            attacks.append((self.attack, self.damage, self.angleX, self.angleY, self.launch, self.sender))
            self.frame -= 1
            if self.frame <= 0:
    ...
    def movement(self, UP, LEFT, DOWN, RIGHT, ATTACK, SHIELD):
        ...
        if self.character == 0:
            if self.state == 1:
                ...
                if self.frame == 10:
                    self.Projectile = self.projectile(self.X, self.Y, 10 * self.facing, 0, 1, 1, 30, self.playerNumber, 10, 10, 5, self.facing, 1)
        ...
        if self.frame == 0:
            ...
            if UP and self.grounded and self.wallGrab == 0: #salto
                ...
                self.flightTimer = 20
                self.wallGrab = 0
            elif (abs(self.VX) < 1) and (CharacterClimb[self.character] == 1) and (not(self.wallGrab == 0)):
                self.VY = (DOWN - UP) * 3
        ...
        self.VY += (1 * CharacterGravity[self.character] * 1.5 if DOWN else 1) if self.wallGrab == 0 else (0.6 * CharacterGravity[self.character]) if CharacterClimb[self.character] == 0 else 0 #gravedad
    ...
    def collide(self):
        ...
        if collided:
            ...
            if self.wallGrab == 0:
                self.VY = 0
            if (self.RIGHT and self.wallStopDir == 1 or self.LEFT and self.wallStopDir == -1) and self.wallGrabTimer == 0 and collided:
