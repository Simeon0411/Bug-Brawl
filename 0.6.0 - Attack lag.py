...
class player():
    ...
    def movement(self, UP, LEFT, DOWN, RIGHT, ATTACK, SHIELD):
        ...
        if ATTACK and self.busyTimer == 0:
            self.Attack = self.attack(self, 0, 10, 100, 100, 10, 0, 0, 0, 0)
            self.busyTimer = 100
        if self.busyTimer == 0:
            if LEFT:
            ...
            if UP and self.grounded: #salto
                ...
                self.flightTimer = 20
                self.wallGrab = 0
            elif UP and (self.wallGrab == 1 or self.wallGrab == -1):
                self.VY = -10 * CharacterJumpHeight[self.character]
                self.VX = -8 if self.RIGHT else 8
                self.jumpTimer = 10
                self.flight = CharacterFlight[self.character]
                self.flightTimer = 20
                self.wallGrab = 0
...
while running:
    ...
    for attack in attacks:
        pygame.draw.rect(screen, (255, 0, 0), attack[0])
    pygame.display.flip()
    print(f"{p1.busyTimer}")
pygame.quit()