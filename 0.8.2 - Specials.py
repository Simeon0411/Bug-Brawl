...
class player():
    ...
    class projectile:
        def __init__(self, X, Y, VX, VY, FX, FY, frame, sender, damage, angleX, angleY, direction, launch, type):
            self.X = X
            self.Y = Y
            self.VX = VX
            self.VY = VY
    ...
    def movement(self, UP, LEFT, DOWN, RIGHT, ATTACK, SHIELD):
        ...
        if self.character == 0:
            if self.state == 1:
                ...
                if self.frame == 10:
                    self.Projectile = self.projectile(self.X, self.Y, 10 * self.facing, 0, 1, 1, 30, self.playerNumber, 10, 10, 5, self.facing, 1, 1)
