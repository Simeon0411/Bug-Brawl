        ...
        def update(self):
            if self.type == 1:
                ...
                if self.frame <= 1:
                    ...
                    self.frame = 0
                    self.projectile = player.projectile(self.X, self.Y - 5, 10, 10, 8, -3, 0.98, 1, 0.5, 10, self.sender, 10, 10, 5, 1, 1, 5, 0, 1)
                    self.projectile = player.projectile(self.X, self.Y - 5, 10, 10, 3, -4, 0.98, 1, 0.5, 10, self.sender, 10, 10, 5, 1, 1, 5, 0, 1)
                    self.projectile = player.projectile(self.X, self.Y - 5, 10, 10, -3, -4, 0.98, 1, 0.5, 10, self.sender, 10, 10, 5, 1, 1, 5, 0, 1)
                    self.projectile = player.projectile(self.X, self.Y - 5, 10, 10, -8, -3, 0.98, 1, 0.5, 10, self.sender, 10, 10, 5, 1, 1, 5, 0, 1)
                # Attack and frame update #
                self.frame -= 1
        ...
        if self.character == 0:
            if self.state == 1:
                ...
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, 20, 0, 30, 15, self.charge + 1, self.charge + 1, 10, 0, self.playerNumber, self.facing, self.hitbox, 0, 0)
                    self.charge = 0
            elif self.state == 2:
                if self.frame > 9 and self.frame < 14:
                    self.Attack = self.attack(self, 20, -5, 40, 20, 10, 30, 10, 0, self.playerNumber, self.facing, self.hitbox, 0, 0)
            elif self.state == 3:
                ...
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, -20, -10, 50, 50, 10, 15, -12, 0, self.playerNumber, self.facing, self.hitbox, 0, 0)
            elif self.state == 6:
                if self.frame > 8 and self.frame < 12:
                ...
                if self.playerNumber == 0:
                    if self.frame == 10:
                        if (RIGHT and self.facing == -1) or (LEFT and self.facing == 1):
                            self.Projectile = self.projectile(self.X, self.Y, 10, 10, 3 * self.facing, -8, 0.98, 1, 0.5, 60, self.playerNumber, 10, 10, 5, self.facing, 1, 2, 0, 0)
                        else:
                            self.Projectile = self.projectile(self.X, self.Y, 10, 10, 8 * self.facing, -5, 0.98, 1, 0.5, 60, self.playerNumber, 10, 10, 5, self.facing, 1, 2, 0, 0)
    ...
    def collide(self):
        ...
        if collided:
            if self.VY > (CharacterGravity[self.character] * 20):
                self.VY *= 0.8
        #Landing lag#
            if self.frame > 0 and self.VY > 3 and self.state != 10 and self.state != 11 and self.state != 12 and self.state != 9:
                self.frame = 8
                self.state = 11
        while any(self.hitbox.colliderect(rect) for rect in block):
...
while running:
    ...
    for pl in players:
        ...
        if not(pl.flight == CharacterFlight[pl.character]) and pl.grounded == 0 and pl.wallGrab == 0:
            pygame.draw.rect(screen, (0, 0, 0), pygame.Rect((pl.X + pl.hitbox.width / 2 - pl.flight - CamX) * CamZ, (pl.Y - 75 / 2 - CamY) * CamZ, (pl.flight * 2) * CamZ, 25 * CamZ))
