...
class player():
    def __init__(self, X, Y, character, playerNumber, sizeX, sizeY):
        ...
        self.dashed = 1
    class attack:
        def __init__(self, pl, attackX, attackY, atkSizeX, atkSizeY, damage, angleX, angleY, launch, sender, direction, hitbox, block, super, hitstun):
    ...
    class projectile:
        ...
        def update(self):
            if self.type == 1:
                ...
                self.frame -= 1
                attacks.append((self.attack, self.damage, self.angleX, self.angleY, self.launch, self.sender, self.block, self.super, self.hitstun))
                if self.frame <= 0:
                ...
                if self.frame <= 3 and self.frame > 0:
                        self.Attack = player.attack(self, -15, -15, 40, 40, 10, 0, 22, 0, self.sender, 1, pygame.Rect(self.X - 20, self.Y - 20, 40, 40), 0, 1, 5)
        ...
        if self.character == 0:
            if self.state == 1:
                ...
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, 20, 0, 30, 15, self.charge + 10, self.charge + 1, 10, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, self.charge + 5)
                    self.charge = 0
            elif self.state == 2:
                if self.frame > 9 and self.frame < 14:
                    self.Attack = self.attack(self, 20, -5, 40, 20, 10, 30, 10, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 8)
            elif self.state == 3:
                if self.frame > 5 and self.frame < 10:
                    self.VX = 7 * self.facing
                if self.frame > 8 and self.frame < 12:
                    self.Attack = self.attack(self, 20, 0, 40, 60, 10, 30, 10, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 8)
                if self.frame < 6:
                ...
                if self.frame > 7 and self.frame < 12:
                    ...
                    self.Attack = self.attack(self, 20, 10, 40, 40, 10, 30, 10, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 8)
                    self.dashing = 0
            elif self.state == 5:
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, -20, -10, 50, 50, 10, 15, -12, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 15)
            elif self.state == 6:
                if self.frame > 8 and self.frame < 12:
                    self.Attack = self.attack(self, -10, 60, 70, 60, 10, 0, 22, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 10)  
            elif self.state == 7:
                if self.frame > 7 and self.frame < 11:
                    self.Attack = self.attack(self, -10, -60, 80, 60, 10, 0, -22, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 10) 
            elif self.state == 8:
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, 30, 0, 70, 60, 10, 22.5, -7.5, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 10)
            elif self.state == 9:
                if self.playerNumber == 0:
                    if self.frame == 10:
