...
class player():
    def __init__(self, X, Y, character, playerNumber):
        ...
        self.superMeter = 0
    class attack:
        def __init__(self, pl, attackX, attackY, atkSizeX, atkSizeY, damage, angleX, angleY, launch, sender, direction, hitbox, block):
            ...
            self.hitbox = hitbox
            self.block = block
            attacks.append((pygame.Rect(self.X + self.attackX, self.Y + self.attackY, self.atkSizeX, self.atkSizeY), self.damage, self.angleX, self.angleY, self.launch, self.sender, self.block)) if direction == 1 else attacks.append((pygame.Rect(self.X - self.attackX - self.atkSizeX + self.hitbox.width, self.Y + self.attackY, self.atkSizeX, self.atkSizeY), self.damage, self.angleX, self.angleY, self.launch, self.sender, self.block))
    class projectile:
        def __init__(self, X, Y, Xsize, Ysize, VX, VY, FX, FY, gravity, frame, sender, damage, angleX, angleY, direction, launch, type, block):
            self.X = X
            self.Y = Y
            self.Xsixe = Xsize
            self.Ysize = Ysize
        ...
        def update(self):
            ...
            if self.VY > 8:
                self.VY = 8
            self.X += self.VX
            self.attack = pygame.Rect(self.X, self.Y, 10, 10)
            ...
            self.attack = pygame.Rect(self.X, self.Y, 10, 10)
            if self.type == 1:
                attacks.append((self.attack, self.damage, self.angleX, self.angleY, self.launch, self.sender, self.block))
            elif self.type ==2:
                attacks.append((self.attack, self.damage, self.angleX, self.angleY, self.launch, self.sender, self.block))
            self.frame -= 1
            if self.type == 2:
                if self.frame <= 3 and self.frame > 0:
                    self.Attack = player.attack(self, -15, -15, 40, 40, 10, 0, 22, 0, self.sender, 1, pygame.Rect(self.X - 20, self.Y - 20, 40, 40), 1)
            if self.type == 4:
                if self.frame == 1:
                    self.Attack = player.attack(self, -15, -15, 40, 40, 10, 0, 22, 0, self.sender, 1, pygame.Rect(self.X - 20, self.Y - 20, 40, 40), 1)
            if self.frame <= 0:
    ...
    def movement(self, UP, LEFT, DOWN, RIGHT, ATTACK, SHIELD, SUPER):
        ...
        self.SHIELD = SHIELD
        self.SUPER = SUPER
        if SUPER and self.superMeter > 25:
            self.state = 10
            self.superMeter -= 25
            self.frame = 20
        if ATTACK and SHIELD and self.frame > (self.maxFrame - 3):
            if self.state != 9:
                self.state = 9
                self.frame = 20
        ...
        if self.character == 0:
            if self.state == 1:
                ...
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, 0, 10, 100, 100, self.charge + 1, self.charge + 1, 10, 0, self.playerNumber, self.facing, self.hitbox, 0)
                    self.charge = 0
            elif self.state == 2:
                if self.frame > 9 and self.frame < 14:
                    self.Attack = self.attack(self, 20, 10, 80, 40, 10, 30, 10, 0, self.playerNumber, self.facing, self.hitbox, 0)
            elif self.state == 3:
                if self.frame > 7 and self.state < 10:
                    self.VX = 7 * self.facing
                    self.Attack = self.attack(self, 20, 0, 40, 60, 10, 30, 10, 0, self.playerNumber, self.facing, self.hitbox, 0)
            elif self.state == 4:
                if self.frame > 7 and self.frame < 12:
                    self.VX = CharacterRunSpeed[self.character] * self.facing
                    self.Attack = self.attack(self, 20, 10, 40, 40, 10, 30, 10, 0, self.playerNumber, self.facing, self.hitbox, 0)
                    self.dashing = 0
            elif self.state == 5:
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, -10, 0, 70, 60, 10, 22.5, 7.5, 0, self.playerNumber, self.facing, self.hitbox, 0)
            elif self.state == 6:
                if self.frame > 8 and self.frame < 12:
                    self.Attack = self.attack(self, -10, 60, 70, 60, 10, 0, 22, 0, self.playerNumber, self.facing, self.hitbox, 0)  
            elif self.state == 7:
                if self.frame > 7 and self.frame < 11:
                    self.Attack = self.attack(self, -10, -60, 80, 60, 10, 0, -22, 0, self.playerNumber, self.facing, self.hitbox, 0) 
            elif self.state == 8:
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, 30, 0, 70, 60, 10, 22.5, 7.5, 0, self.playerNumber, self.facing, self.hitbox, 0)
            elif self.state == 9:
                if self.playerNumber == 0:
                    if self.frame == 10:
                        self.Projectile = self.projectile(self.X, self.Y, 10, 10, 8 * self.facing, -3, 0.98, 1, 0.5, 60, self.playerNumber, 10, 10, 5, self.facing, 1, 2, 0)
                elif self.playerNumber == 1:
                    if self.frame == 15:
                        self.Projectile = self.projectile(self.X, self.Y, 10, 10, 8 * self.facing, 0, 1, 0, 0, 30, self.playerNumber, 10, 10, 5, self.facing, 1, 1, 0)
            elif self.state == 10:
                if self.playerNumber == 0:
                    if self.frame == 10:
                        self.Projectile = self.projectile(self.X, self.Y, 20, 20, 8 * self.facing, -3, 0.98, 1, 0.5, 60, self.playerNumber, 20, 10, 5, self.facing, 1, 4, 0)
                elif self.playerNumber == 1:
                    if self.frame == 15:
                        self.Projectile = self.projectile(self.X, self.Y, 20, 20, 8 * self.facing, 0, 1, 0, 0, 30, self.playerNumber, 20, 10, 5, self.facing, 1, 3, 0)
        ...
        self.VY += (1 * CharacterGravity[self.character] * 1.5 if DOWN else 1) if self.wallGrab == 0 else (0.6 * CharacterGravity[self.character]) if (CharacterClimb[self.character] == 0 and self.wallGrab != 0) else 0 #gravedad
        ...
        for attack in attacks:
            if self.hitbox.colliderect(attack[0]) and not(attack[5] == self.playerNumber):
                if self.gotHit == 0 and self.DgotHit == 0 or (attack[6] == 1 and self.DgotHit == 0):
                    ...
                    self.gotHit += 1
                players[attack[5]].superMeter += round(attack[1] * 0.5)
...
while running:
    ...
    for pl in players:
        if pl.playerNumber == 0:
            pl.movement(keys[pygame.K_w], keys[pygame.K_a], keys[pygame.K_s], keys[pygame.K_d], keys[pygame.K_f], keys[pygame.K_g], keys[pygame.K_c])
