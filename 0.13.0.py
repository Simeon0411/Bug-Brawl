...
pygame.init()
#player stats#
CharacterWalkSpeed = [2, 1.7]
CharacterRunSpeed = [4, 3.5]
CharacterAirSpeed = [0.5, 0.6]
CharacterJumpHeight = [0.7, 0.8]
CharacterGravity = [0.7, 0.7]
CharacterFlight = [30, 0]
CharacterClimb = [0, 0]
CharacterHP = [100, 80]
running = True
...
class player():
    def __init__(self, X, Y, character, playerNumber):
        ...
        self.VX = 0
        self.VY = 0
        self.sizeX = 35 if self.character == 0 else 50
        self.sizeY = 20 if self.character == 0 else 40
    ...
    def movement(self, UP, LEFT, DOWN, RIGHT, ATTACK, SHIELD, SUPER, SPECIAL):
        ...
        if ATTACK and self.frame == 0 and self.wallGrab == 0:
            if self.grounded == 1 and self.frame == 0:
                # Jab #
                ...
                # Dash attack #
                    ...
                    self.maxFrame = 20
                    self.state = 4
            elif self.grounded != 1 and self.frame == 0:
                # Nair #
        ...
        if self.character == 0:
            if self.state == 1:
                ...
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, 20, 0, 30, 15, self.charge + 10, self.charge + 1, 6, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, self.charge + 5, 0)
                    self.charge = 0
            elif self.state == 2:
                if self.frame > 9 and self.frame < 14:
                    self.Attack = self.attack(self, 20, -5, 40, 20, 10, 20, 8, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 8, 0)
            elif self.state == 3:
                if self.frame > 5 and self.frame < 10:
                    self.VX = 7 * self.facing
                if self.frame > 8 and self.frame < 12:
                    self.Attack = self.attack(self, 20, 0, 40, 60, 10, 15, 4, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 8, 0)
                if self.frame < 6:
                ...
                if self.frame > 7 and self.frame < 12:
                    ...
                    self.Attack = self.attack(self, 20, 10, 40, 40, 10, 24, 10, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 8, 0)
                    self.dashing = 0
            elif self.state == 5:
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, -20, -10, 50, 50, 10, 10, -10, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 18, 0)
            elif self.state == 6:
                if self.frame > 5 and self.frame < 9:
                    self.Attack = self.attack(self, 0, 20, 30, 40, 10, 0, 12, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 25, 0)  
            elif self.state == 7:
                if self.frame > 3 and self.frame < 7:
                    self.Attack = self.attack(self, -25, -30, 80, 40, 18, 0, -16, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 10, 0) 
            elif self.state == 8:
                if self.frame > 9 and self.frame < 12:
                    self.Attack = self.attack(self, 30, 0, 40, 30, 10, 22.5, -7.5, 0, self.playerNumber, self.facing, self.hitbox, 0, 0, 10, 0)
            elif self.state == 9:
                if self.frame == 30:
        ...
        if self.frame == 0:
            ...
            self.frame -= 1
        # Walk #
        if not(DOWN and self.grounded == 1) and (self.state == 0 or self.state == 8):
        ...
        # Gravity #
        if self.state != 11 and self.state != 13:
    ...
    def collide(self):
        ...
        # Wall bounce #
        if collided:
            if (self.VX > 10 or self.VX < -10) and self.state == 11:
                self.VX *= -0.8
            else:
                self.VX = 0
            # Wall grab gravity #
        ...
        for p in homing:
            if p[2] == homing[0][2]:
                if p[3] > 0:
                    ...
                    homing.remove(p)
players = [player(200, 0, int(input("Player 0 character(0-1): ")), 0), player(600, 0, int(input("Player 1 character(0-1): ")), 1)]
while running:
